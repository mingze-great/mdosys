...
package com.mdosys.scheduler.service.registry;
...
import com.mdosys.scheduler.common.utils.HeartBeat;
...
public class RegistryClient {
    ...
    public void handleDeadServer(Collection<String> nodes, NodeType nodeType, String opType) {
        nodes.forEach(node -> {
            ...
            if (opType.equals(DELETE_OP)) {
                ...
                String deadServerPath = REGISTRY_SCHEDULER_DEAD_SERVERS + SINGLE_SLASH + type + UNDERLINE + host;
    ...
    public boolean checkIsDeadServer(String node, String serverType) {
        ...
        String deadServerPath = REGISTRY_SCHEDULER_DEAD_SERVERS + SINGLE_SLASH + serverType + UNDERLINE
                + ipSeqNo;
    ...
    public Collection<String> getMasterNodesDirectly() {
        return getChildrenKeys(REGISTRY_SCHEDULER_MASTERS);
    }

    public Collection<String> getWorkerGroupDirectly() {
        return getChildrenKeys(REGISTRY_SCHEDULER_WORKERS);
    }

    public Collection<String> getWorkerGroupNodesDirectly(String workerGroup) {
        return getChildrenKeys(REGISTRY_SCHEDULER_WORKERS + "/" + workerGroup);
    }
    ...
    public boolean isMasterPath(String path) {
        return path != null && path.startsWith(REGISTRY_SCHEDULER_MASTERS);
    }

    public boolean isWorkerPath(String path) {
        return path != null && path.startsWith(REGISTRY_SCHEDULER_WORKERS);
    }
    ...
    private void initNodes() {
        registry.put(REGISTRY_SCHEDULER_MASTERS, EMPTY, false);
        registry.put(REGISTRY_SCHEDULER_WORKERS, EMPTY, false);
        registry.put(REGISTRY_SCHEDULER_DEAD_SERVERS, EMPTY, false);
    }
    ...
    private void removeDeadServerByHost(String host, String serverType) {
        Collection<String> deadServers = getChildrenKeys(REGISTRY_SCHEDULER_DEAD_SERVERS);
        for (String serverPath : deadServers) {
            if (serverPath.startsWith(serverType + UNDERLINE + host)) {
                String server = REGISTRY_SCHEDULER_DEAD_SERVERS + SINGLE_SLASH + serverPath;
                remove(server);
