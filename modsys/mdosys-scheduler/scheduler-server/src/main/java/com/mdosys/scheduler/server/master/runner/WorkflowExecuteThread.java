...
import com.mdosys.scheduler.common.graph.DAG;
import com.mdosys.scheduler.common.model.TaskNode;
import com.mdosys.scheduler.common.model.TaskNodeRelation;
import com.mdosys.scheduler.common.process.ProcessDag;
import com.mdosys.scheduler.common.process.Property;
...
import com.mdosys.scheduler.dao.utils.DagHelper;
import com.mdosys.scheduler.remote.command.HostUpdateCommand;
...
import com.mdosys.scheduler.server.master.runner.task.ITaskProcessor;
import com.mdosys.scheduler.server.master.runner.task.TaskAction;
import com.mdosys.scheduler.server.master.runner.task.TaskProcessorFactory;
import com.mdosys.scheduler.service.alert.ProcessAlertManager;
import com.mdosys.scheduler.service.process.ProcessService;
import com.mdosys.scheduler.service.quartz.cron.CronUtils;
import com.mdosys.scheduler.service.queue.PeerTaskInstancePriorityQueue;
...
import static com.mdosys.scheduler.common.Constants.*;
...
public class WorkflowExecuteThread implements Runnable {
     ...
     */
    public WorkflowExecuteThread(ProcessInstance processInstance, ProcessService processService,
            NettyExecutorManager nettyExecutorManager, ProcessAlertManager processAlertManager,
            MasterConfig masterConfig, ConcurrentHashMap<Integer, TaskInstance> taskTimeoutCheckList,
    ...
    private boolean taskTimeout(StateEvent stateEvent) {
        ...
        TaskDefinition taskDefinition = processService.findTaskDefinition(taskInstance.getTaskCode(),
    ...
    private void taskFinished(TaskInstance task) {
        ...
        if (task.taskCanRetry()) {
            ...
            if (!task.retryTaskIntervalOverTime()) {
                logger.info(
                        "failure task will be submitted: process id: {}, task instance id: {} state:{} retry times:{} / {}, interval:{}",
                        processInstance.getId(),
                        task.getId(),
                        task.getState(),
                        task.getRetryTimes(),
    ...
    private boolean processStateChangeHandler(StateEvent stateEvent) {
        try {
            logger.info("process:{} state {} change to {}", processInstance.getId(), processInstance.getState(),
    ...
    private void buildFlowDag() throws Exception {
        ...
        List<TaskNode> taskNodeList = processService.transformTask(
                processService.findRelationByCode(processDefinition.getProjectCode(), processDefinition.getCode()),
                Lists.newArrayList());
    ...
    private TaskInstance submitTaskExec(TaskInstance taskInstance) {
        try {
            ...
            boolean submit = taskProcessor.submit(taskInstance, processInstance,
                    masterConfig.getMasterTaskCommitRetryTimes(), masterConfig.getMasterTaskCommitInterval());
            if (submit) {
    ...
    private void submitPostNode(String parentNodeCode) {
        Set<String> submitTaskNodeList = DagHelper.parsePostNodes(parentNodeCode, skipTaskNodeList, dag,
                completeTaskList);
        ...
        for (TaskInstance task : taskInstances) {
            ...
            if (task.getState().typeIsPause() || task.getState().typeIsCancel()) {
                logger.info("task {} stopped, the state is {}, task id:{}", task.getName(), task.getState(),
                        task.getId());
            } else {
                addTaskToStandByList(task);
            }
    ...
    private boolean dependTaskSuccess(String dependNodeName, String nextNodeName) {
        if (dag.getNode(dependNodeName).isConditionsTask()) {
            ...
            List<String> nextTaskList = DagHelper.parseConditionTask(dependNodeName, skipTaskNodeList, dag,
                    completeTaskList);
    ...
    private void addTaskToStandByList(TaskInstance taskInstance) {
        logger.info("add task to stand by list, task name: {} , task id:{}", taskInstance.getName(),
                taskInstance.getId());
        try {
            ...
            logger.error("add task instance to readyToSubmitTaskQueue, taskName: {}, task id: {}",
                    taskInstance.getName(), taskInstance.getId(), e);
        }
    }
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (task.taskCanRetry()) {
                    ...
                    if (retryTask != null && retryTask.getState().equals(ExecutionStatus.FORCED_SUCCESS)) {
                        ...
                        logger.info(
                                "task name: {} has been forced success, put it into complete task list and stop retrying, task id:{}",
                ...
                if (DependResult.SUCCESS == dependResult) {
                    ...
                    removeTaskFromStandbyList(task);
                    logger.info("remove task {},id:{} , because depend result : {}", task.getName(), task.getId(),
                            dependResult);
                }
            }
        } catch (Exception e) {
