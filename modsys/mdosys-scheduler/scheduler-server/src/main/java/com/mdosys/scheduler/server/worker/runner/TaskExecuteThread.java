...
public class TaskExecuteThread implements Runnable, Delayed {
    ...
    public void run() {
        ...
        try {
            ...
            if (null == taskChannel) {
                throw new RuntimeException(String.format("%s Task Plugin Not Found,Please Check Config File.",
        ...
        for (Map.Entry<String, String> resource : resEntries) {
            ...
            if (!resFile.exists()) {
                try {
                    ...
                    HadoopUtils.getInstance().copyHdfsToLocal(resHdfsPath, execLocalPath + File.separator + fullName,
                            false, true);
                } catch (Exception e) {
                    logger.error(e.getMessage(), e);
