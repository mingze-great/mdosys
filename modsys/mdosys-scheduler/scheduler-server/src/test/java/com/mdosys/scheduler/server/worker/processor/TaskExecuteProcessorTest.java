...
import com.mdosys.scheduler.common.utils.FileUtils;
...
import com.mdosys.scheduler.remote.utils.ChannelUtils;
import com.mdosys.scheduler.remote.utils.JsonSerializer;
...
public class TaskExecuteProcessorTest {
        ...
        public void before() throws Exception {
                ...
                PowerMockito.doNothing().when(taskCallbackService).sendAck(taskExecutionContext.getTaskInstanceId(),
                                ackCommand);
                ...
                PowerMockito.when(workerManager.offer(
                                new TaskExecuteThread(taskExecutionContext, taskCallbackService, alertClientService)))
                ...
                PowerMockito.when(JSONUtils.parseObject(command.getBody(), TaskExecuteRequestCommand.class))
                                .thenReturn(taskRequestCommand);
                PowerMockito.when(JSONUtils.parseObject(taskRequestCommand.getTaskExecutionContext(),
                ...
                PowerMockito.doNothing().when(FileUtils.class, "createWorkDirIfAbsent",
        ...
        private static class SimpleTaskExecuteThread extends TaskExecuteThread {
                ...
                public SimpleTaskExecuteThread(TaskExecutionContext taskExecutionContext,
                                TaskCallbackService taskCallbackService, Logger taskLogger,
